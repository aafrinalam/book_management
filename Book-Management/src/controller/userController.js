...
const createUser = async function (req, res) {
  ...
  try {
    if (Object.keys(req.body).length == 0)
      return res
        .status(400)
        .send({ status: false, msg: "please enter the details of the user" });
    ...
    if (!userData.title)
      return res
        .status(400)
        .send({ status: false, msg: `title of the user is not present` });
    if (userData.title.trim().length == 0)
      return res
        .status(400)
        .send({ status: false, msg: "enter the title in proper format" });
    // if title is present should be from ["Mr", "Mrs", "Miss"]
    let title = ["Mr", "Mrs", "Miss"];
    if (!title.includes(userData.title.trim()))
      return res
        .status(400)
        .send({ status: false, msg: "title should be from [Mr, Mrs, Miss]" });
    //check name
    if (!userData.name)
      return res
        .status(400)
        .send({ status: false, msg: "name of the user is not present" });
    if (userData.name.trim().length == 0)
      return res
        .status(400)
        .send({ status: false, msg: "enter the name in proper format" });
    //check phone number
    if (!userData.phone)
      return res
        .status(400)
        .send({ status: false, msg: "phone no. of the user is not present" });
    // validation of phone number
    if (!/^([+]\d{2})?\d{10}$/.test(userData.phone.trim())) {
      return res
        .status(400)
        .send({ status: false, msg: "phone no. is not valid" });
    }
    ...
    if (dupPhone)
      return res
        ...
        .send({
          status: false,
          msg: `${userData.phone} is already registered`,
        });
    //check email
    if (!userData.email)
      return res
        .status(400)
        .send({ status: false, msg: "email of the user is not present" });
    // validation of email
    if (
      !/^\w+([\.-]?\w+)@\w+([\. -]?\w+)(\.\w{2,3})+$/.test(
        userData.email.trim()
      ...
      return res
        .status(400)
        .send({ status: false, msg: "email ID is not valid" });
    ...
    if (dupEmail)
      return res
        ...
        .send({
          status: false,
          msg: `${userData.email} is already registered`,
        });
    // check password
    if (!userData.password)
      return res
        .status(400)
        .send({ status: false, msg: "password of the user is not present" });
...
const loginUser = async function (req, res) {
  try {
    ...
    if (!userName || !password)
      return res
        .status(400)
        .send({ status: false, msg: "username or password is not present" });
    ...
    if (!user)
      return res
        ...
        .send({
          status: false,
          msg: " EmailId or the password is not correct",
        });
    let token = jwt.sign(
      {
